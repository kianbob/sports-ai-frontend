...
export default function ChatInterface() {
  ...
  const sendMessage = async (messageText?: string) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: 'assistant', content: 'Sorry, error.', timestamp: new Date() }]);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex-1 flex flex-col">
            ...
            {messages.length === 0 && (
              <div className="text-center py-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-2xl mx-auto">
                    {['Show me NFL odds', 'Show me Patrick Mahomes profile', 'Show me Chiefs injuries'].map((s, i) => (
                        <button key={i} onClick={() => { setInput(s); sendMessage(s); }} className="p-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-left text-gray-300 transition-colors">{s}</button>
                    ))}
                </div>
              </div>
            )}
        ...
        <div className="bg-black/30 backdrop-blur-lg border-t border-white/10 p-4 sticky bottom-0">
          <div className="max-w-4xl mx-auto flex gap-2">
            ...
            <button onClick={() => sendMessage()} disabled={loading || !input.trim()} className="px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl font-semibold"><PaperAirplaneIcon className="w-5 h-5" /></button>
          </div>
        </div>
      </div>
    </div>
